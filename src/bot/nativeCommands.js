...
╰━━━━━━━━━━━━━━━╯`);
  ...
  async play({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      let result;
      try {
        result = await downloader.youtubeAudio(query);
      } catch (e1) {
        // Fallback: tenta baixar direto via stream pra buffer
        try {
          await reply(sock, msg, ctx, '⏳ APIs falharam, tentando download direto (pode demorar)...');
          const r2 = await downloader.youtubeAudioBuffer(query);
          await sock.sendMessage(ctx.remoteJid, {
          ...
          throw new Error('APIs: ' + e1.message.slice(0,100) + ' | Stream: ' + e2.message.slice(0,100));
        }
      }

      // Tenta enviar pela URL direta
      try {
        await sock.sendMessage(ctx.remoteJid, {
          audio: { url: result.url }, mimetype: 'audio/mpeg', fileName: `${result.title}.mp3`,
        }, { quoted: msg });
      } catch (sendErr) {
        // Se falhar URL, baixa pra buffer e tenta de novo
  ...
  async video({ sock, msg, ctx, args }) {
    ...
    try {
      ...
      try {
        ...
        await sock.sendMessage(ctx.remoteJid, { video: buf, caption: `🎬 *${r.title}*` }, { quoted: msg });
      }
      await react(sock, msg, '✅');
